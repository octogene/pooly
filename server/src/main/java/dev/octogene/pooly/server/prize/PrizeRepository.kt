...
interface PrizeRepository {
    suspend fun getAllPrizes(wallets: List<Address>): Either<DatabaseError, List<Prize>>
    suspend fun getLatestPrizes(wallets: List<Address>): Either<DatabaseError, List<Prize>>
}
...
class PrizeRepositoryImpl(
    ...
    override suspend fun getAllPrizes(wallets: List<Address>): Either<DatabaseError, List<Prize>> = either {
        suspendTransaction(database) {
            Prizes
                .selectAll()
                .where { Prizes.winnerAddress inList wallets.map { it.value } }
